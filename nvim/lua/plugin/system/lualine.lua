	...
	if mode:match("[vV\022]") then
		...
		if mode == "\022" then
			...
			for _, text in ipairs(lines) do
				...
				if s_col <= e_col then
					-- count = count + (e_col - s_col + 1) -- LF, Newline break character
				end
			end
		elseif mode == 'V' then
			-- Visual Line Mode
			...
			-- count = count + (#lines - 1) -- LF, Newline break character
		else
			-- Visual Mode
		...
		select_info = string.format(" Select(%d)", count)
	end

	return string.format("%d:%d%s", line, vcol, select_info)
end
